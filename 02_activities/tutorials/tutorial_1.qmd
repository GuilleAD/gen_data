...
P - Hardy-Weinberg p-value testing deviation from equilibrium. We really only care about this column for QC purposes. A very low p-value (e.g., \< 1e-6) may indicate genotyping error or population stratification, while a high p-value suggests the SNP is in HWE. A low P value does not have a definitive explanation without further investigation, genorying error is one possibility, but it could also be due to true biological effects.
